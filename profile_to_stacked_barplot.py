...
def melt_feature_table(profile_df, metadata_df):
	...
	melt_profile_df = pd.melt(
		profile_df, 
		id_vars=[sample_col], 
		value_vars=features_col, 
		var_name="Features"
	)
...
def define_layout(fig):
	#Define font formats
	title_font_format = dict(
		size=20,
		family="Open Sans",
		color="#2e1c18"
	)

	axis_font_format = dict(
		size=12,
		family="Open Sans",
		color="#2e1c18"
	)

	legend_font_format = dict(
		size=8,
		family="Open Sans",
		color="#2e1c18"
	)
	...
	fig.update_layout(
		...
		title=dict(
			text="Taxonomy Relative Abundance Barplot",
			x=0.5,
			font=title_font_format
		),
...
def create_stacked_barplot(profile_df):
	...
	fig = px.bar(
		...
		y="value", 
		color="Features",
		color_discrete_sequence =col_scale_features
	)
...
def main(args):
	...
	#fig.write_html("./try_export_file.html")
...
if __name__ == "__main__":
	...
	required_args.add_argument(
		...
		required=True,
		metavar="STRING",
		help="Path to profile table in TSV format"
	)
