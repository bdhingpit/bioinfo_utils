...
def merge_contig_tables(abund_table, annot_table, will_split_lineage):
	...
	#Merge
	merged_df = abund_df.merge(
		annot_df.iloc[:, list(range(0, 4)) + [-1]], 
		left_on='Contig',
		right_on=0
	)

	#Rearrange the columns
	merged_df = merged_df.iloc[:, [0] + list(range(abund_df.shape[1]+1, merged_df.shape[1])) + list(range(1, abund_df.shape[1]))]
...
# **Add other options which gene groups (KO, COG, etc) to use
def parse_eggnog_table(annot_table):
	annot_df = pd.read_csv(annot_table, sep="\t", header=4)

	annot_df = annot_df[["#query", "KEGG_ko"]]
	annot_df["KEGG_ko"] = annot_df["KEGG_ko"].str.split(",", expand=True)[0] #If multiple KOs are present, take the first one only
...
if __name__ == '__main__':
	...
	required_args.add_argument(
		...
		required=True,
		metavar='PATH',
		help='Path to the abundance table input'
	)
	...
	required_args.add_argument(
		...
		required=True,
		metavar='PATH',
		help='Path to the annotation table input'
	)
	...
	required_args.add_argument(
		...
		required=True,
		metavar='PATH',
		help='Path to output merged table'
	)
